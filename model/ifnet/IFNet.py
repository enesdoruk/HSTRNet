...
from model.ifnet.refine import *
...
class IFNet(nn.Module):
    ...
    def forward(self, x, scale=[4,2,1], timestep=0.5):
        ...
        for i in range(3):
            ...
            warped_img0 = warp(img0, flow[:, :2], device)
            warped_img1 = warp(img1, flow[:, 2:4], device)
            merged_student = (warped_img0, warped_img1)
            merged.append(merged_student)
        if gt.shape[1] == 3:
            flow_d, mask_d = self.block_tea(torch.cat((img0, img1, warped_img0, warped_img1, mask, gt), 1), flow, scale=1)
            flow_teacher = flow + flow_d
            warped_img0_teacher = warp(img0, flow_teacher[:, :2], device)
            warped_img1_teacher = warp(img1, flow_teacher[:, 2:4], device)
