...
class DDT(nn.Module):
    def __init__(self, 
        ...
        self.encoder_level1 = nn.Sequential(*[TransformerBlock(dim=dim*2, num_heads=heads[0], 
                                                               num_groups=groups[0], ffn_expansion_factor=ffn_expansion_factor, 
                                                               bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[16,16],
                                                               grid_size=[16,16], activate=activate) for i in range(num_blocks[0])])
        
        self.down1_2 = Downsample(dim*3) ## From Level 1 to Level 2
        self.encoder_level2 = nn.Sequential(*[TransformerBlock(dim=int(dim*3), num_heads=heads[1], 
                                                               num_groups=groups[1], ffn_expansion_factor=ffn_expansion_factor, 
                                                               bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[16,16],
                                                               grid_size=[16,16], activate=activate) for i in range(num_blocks[1])])
        
        self.down2_3 = Downsample(int(dim*5)) ## From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(*[TransformerBlock(dim=int(dim*5), num_heads=heads[2], 
                                                               num_groups=groups[2], ffn_expansion_factor=ffn_expansion_factor,
                                                               bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[8,8],
                                                               grid_size=[8,8], activate=activate) for i in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim*9)) ## From Level 3 to Level 4
        self.latent = nn.Sequential(*[TransformerBlock(dim=int(dim*9), num_heads=heads[3], 
                                                       num_groups=groups[3], ffn_expansion_factor=ffn_expansion_factor, 
                                                       bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[8,8],
                                                       grid_size=[8,8], activate=activate) for i in range(num_blocks[3])])
        
        self.up4_3 = Upsample(int(dim*17)) ## From Level 4 to Level 3
        # self.reduce_chan_level3 = nn.Conv2d(int(dim*2**3), int(dim*2**2), kernel_size=1, bias=bias)
        self.decoder_level3 = nn.Sequential(*[TransformerBlock(dim=int(dim*17), num_heads=heads[2],
                                                                num_groups=groups[2], ffn_expansion_factor=ffn_expansion_factor, 
                                                                bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[8,8],
                                                                grid_size=[8,8], activate=activate) for i in range(num_blocks[2])])


        self.up3_2 = Upsample(int(dim*22)) ## From Level 3 to Level 2
        self.reduce_chan_level2 = nn.Conv2d(int(dim*22), int(dim*4), kernel_size=1, bias=bias)
        self.decoder_level2 = nn.Sequential(*[TransformerBlock(dim=int(dim*4), num_heads=heads[1], 
                                                               num_groups=groups[1], ffn_expansion_factor=ffn_expansion_factor, 
                                                               bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[16,16],
                                                               grid_size=[16,16], activate=activate) for i in range(num_blocks[1])])
        
        self.up2_1 = Upsample(int(dim*7))  ## From Level 2 to Level 1  (NO 1x1 conv to reduce channels)
        self.reduce_chan_level1 = nn.Conv2d(int(dim*7), int(dim*2), kernel_size=1, bias=bias)
        ...
        self.up2_0 = Upsample(int(dim*4))  ## From Level 2 to Level 1  (NO 1x1 conv to reduce channels)
        self.reduce_chan_level0 = nn.Conv2d(int(dim*4), int(dim*1), kernel_size=1, bias=bias)
        self.decoder_level0 = nn.Sequential(*[TransformerBlock(dim=int(dim*1), num_heads=heads[0],
                                                               num_groups=groups[0], ffn_expansion_factor=ffn_expansion_factor, 
                                                               bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[16,16],
                                                               grid_size=[16,16], activate=activate) for i in range(num_blocks[0])])
        

        self.proj_fit = nn.Conv2d(dim, 4, 3, 2, 1)
        self.refinement = nn.Sequential(*[TransformerBlock(dim=int(dim**0.5), num_heads=heads[0], num_groups=groups[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, patch_size=[16,16],grid_size=[16,16], activate=activate) for i in range(num_refinement_blocks)])
        

        self.output = nn.Conv2d(int(dim**0.5), out_channels, kernel_size=3, stride=1, padding=1, bias=bias)
        self.padder_size = 64
    ...
    def forward(self, inp_img, warped_gt, c0):
        B, C, H, W = inp_img.shape
        inp_img = self.check_image_size(inp_img)
        
        inp_enc_level1 = self.patch_embed(torch.cat((inp_img, warped_gt), 1))
        out_enc_level1 = self.encoder_level1(inp_enc_level1) # (32, 256, 256)

        inp_enc_level2 = self.down1_2(torch.cat((out_enc_level1, c0[0]), 1))
        out_enc_level2 = self.encoder_level2(inp_enc_level2) # (48, 128, 128)

        inp_enc_level3 = self.down2_3(torch.cat((out_enc_level2, c0[1]), 1))
        out_enc_level3 = self.encoder_level3(inp_enc_level3) # (80, 64, 64)

        inp_enc_level4 = self.down3_4(torch.cat((out_enc_level3, c0[2]), 1))        
        latent = self.latent(inp_enc_level4) # (144, 32, 32)


        inp_dec_level3 = self.up4_3(torch.cat((latent, c0[3]), 1))
        # inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
        out_dec_level3 = self.decoder_level3(inp_dec_level3) # (272, 64, 64)

        inp_dec_level2 = self.up3_2(torch.cat((out_dec_level3, out_enc_level3), 1))
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        out_dec_level2 = self.decoder_level2(inp_dec_level2) # (64, 128, 128)

        inp_dec_level1 = self.up2_1(torch.cat((out_dec_level2, out_enc_level2), 1))
        inp_dec_level1 = self.reduce_chan_level1(inp_dec_level1)
        out_dec_level1 = self.decoder_level1(inp_dec_level1) # (32, 256, 256)

        inp_dec_level0 = self.up2_0(torch.cat((out_dec_level1, out_enc_level1), 1))
        inp_dec_level0 = self.reduce_chan_level0(inp_dec_level0)
        out_dec_level0 = self.decoder_level0(inp_dec_level0) # (16, 512, 512)

        out_fit = self.proj_fit(out_dec_level0)

        out = self.refinement(out_fit)

        # we have 6 channel inp+warp, output has 4 channel. 
        out[:,:3,:,] = self.output(out)[:,:3,:,] + inp_img

        out = out[:,:,0:H,0:W]
        return out

model = DDT(dim = 16).cuda()

lr = torch.rand((1, 3, 256, 256)).cuda()
warped_gt = torch.rand((1,3,256,256)).cuda()
c0 = [torch.rand((1,16,256,256)).cuda(),
      torch.rand((1,32,128,128)).cuda(),
      torch.rand((1,64,64,64)).cuda(),
      torch.rand((1,128,32,32)).cuda()]

model(lr, warped_gt, c0)
