...
def train(model):
    ...
    dataset_train = VizdroneDataset("train", "/home/other/Umut/Vizdrone/upsampled/original/", device)
    ...
    dataset_val = VizdroneDataset("validation", "/home/other/Umut/Vizdrone/upsampled/original/", device)
    ...
    model.ifnet.load_state_dict(torch.load('./model_dict/vizdrone/HSTR_ifnet_0.pkl', map_location=device))
   
    model.contextnet.load_state_dict(torch.load('./model_dict/vizdrone/HSTR_contextnet_0.pkl', map_location=device))

    model.attention.load_state_dict(torch.load('./model_dict/vizdrone/HSTR_attention_0.pkl', map_location=device))

    model.unet.load_state_dict(torch.load('./model_dict/vizdrone/HSTR_unet_0.pkl', map_location=device))
    ...
    for epoch in range(1, 100):
        ...
        for trainIndex, data in enumerate(tqdm(train_data)):
            ...
            if trainIndex == (train_data.__len__() - 1):
                torch.save(model.ifnet.state_dict(), "model_dict/vizdrone/HSTR_ifnet_" + str(epoch) + ".pkl")
                torch.save(model.contextnet.state_dict(), "model_dict/vizdrone/HSTR_contextnet_" + str(epoch) + ".pkl")
                torch.save(model.attention.state_dict(), "model_dict/vizdrone/HSTR_attention_" + str(epoch) + ".pkl")
                torch.save(model.unet.state_dict(), "model_dict/vizdrone/HSTR_unet_" + str(epoch) + ".pkl")
                torch.save(optimizer.state_dict(), "model_dict/vizdrone/HSTR_optimizer_" + str(epoch) + ".pkl")
...
if __name__ == "__main__":
    ...
    log_file_path = "logs/logs_vizdrone/training.log" + str(datetime.datetime.today().strftime('_%d-%m-%H'))
